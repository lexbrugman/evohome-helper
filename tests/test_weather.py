...
async def test_get_current_temperature_returns_none_when_entity_missing(monkeypatch):
...
async def test_get_current_temperature_returns_none_on_failure(monkeypatch):
    monkeypatch.setattr("settings.HOMEASSISTANT_AUTO_ECO_WEATHER_ENTITY", "weather.home")
...
async def test_get_current_temperature_returns_none_when_attribute_missing(monkeypatch):
    monkeypatch.setattr("settings.HOMEASSISTANT_AUTO_ECO_WEATHER_ENTITY", "weather.home")

    with aioresponses() as m:
        m.get("http://ha.local/api/states/weather.home", payload={"attributes": {}})
        assert await weather.get_current_temperature() is None
